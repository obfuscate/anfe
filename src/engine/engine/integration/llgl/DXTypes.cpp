...
#include <engine/integration/llgl/DXTypes.h>
...
#define LLGL_TRAP_DX_MAP(TYPE, VALUE, DXTYPE) \
    engine::logger().error(fmt::format("failed to map LLGL::{}({}) to {} Direct3D parameter", #TYPE, static_cast<int>(VALUE), #DXTYPE))

#define LLGL_TRAP_DX_MAP_NOVALUE(TYPE, DXTYPE) \
    engine::logger().error(fmt::format"failed to map LLGL::{} to {} Direct3D parameter", #TYPE, #DXTYPE))

#define LLGL_TRAP_DX_UNMAP(TYPE, DXTYPE, DXVALUE) \
    engine::logger().error(fmt::format"failed to unmap LLGL::{} from {} Direct3D parameter ({})", #TYPE, static_cast<unsigned>(DXVALUE)))

#define LLGL_TRAP_DX_PARAM_UNSUPPORTED(PARAM, REQUIREMENT) \
    engine::logger().error(fmt::format"parameter '{}' requires {}", PARAM, REQUIREMENT))
